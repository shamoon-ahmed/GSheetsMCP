...
def ensure_poster_table_exists():
    ...
    Creates the table if it doesn't exist. Preserves all existing records.
    ...
    try:
        ...
        # Create table only if it doesn't exist (preserves existing records)
        cursor.execute("""
        ...
        cursor.execute("""
            ...
            ON poster_generations(tenant_id, created_at DESC);
        """)
        
        conn.commit()
        logger.info("Poster generations table ensured to exist with correct schema")
...
@mcp.tool()
def generate_and_upload_poster_tool(
    prompt: str, 
    product_image_url: str = "", 
    tenant_id: str = None
) -> str:
    """
    Generate marketing poster with Gemini API and upload to ImageKit CDN in one seamless flow.
    
    This unified tool:
    1. Generates poster image using gemini-2.5-flash-image
    2. Converts image to base64 IN MEMORY (no local file saving)
    3. Uploads directly to ImageKit CDN
    4. Saves metadata to Neon Postgres database
    5. Returns ImageKit CDN URL with AI-generated caption
    
    Parameters:
    - prompt: Marketing prompt for poster generation
    - product_image_url: Optional product image URL from Media column (fetched and included in generation)
    - tenant_id: User/session identifier (auto-generated if not provided)
    
    Returns:
    - imagekit_url: CDN URL ready to use
    - caption: Social media caption with hashtags (auto-generated by Gemini)
    - tenant_id: Session identifier for tracking
    - No local files created (production-ready for deployment)
    """
    logger.info(f"Marketing: Starting unified poster generation and upload flow")
...
(Just like that - nothing before the actual caption)"""
            ...
            try:
                logger.info(f"Marketing: Trying model {primary_model}")
                response = client.models.generate_content(
                    model=primary_model,
                    contents=contents,
                )
                model_used = primary_model
                logger.info(f"Marketing: Successfully used model {primary_model}")
            except Exception as model_error:
                logger.warning(f"Marketing: Model {primary_model} failed: {model_error}")
            ...
            # Process Gemini response - extract image and caption
            image_binary = None
            text_response = ""
            ...
            for part in response.candidates[0].content.parts:
                if part.text is not None:
                    ...
                    # Store image binary data in memory (never save to disk)
                    image_binary = part.inline_data.data
                    logger.info("Marketing: Received image binary from Gemini")
            ...
            # Convert image binary to base64 IN MEMORY (ImageKit expects base64 string)
            logger.info("Marketing: Converting image to base64 for ImageKit upload")
            img_base64 = base64.b64encode(image_binary).decode('utf-8')
            
            # Parse social media caption from Gemini response
            ...
            # Use AI-generated caption from Gemini
            final_caption = social_media_caption
            
            # Initialize ImageKit client
            logger.info("Marketing: Initializing ImageKit client")
            ...
            if not all([imagekit_private_key, imagekit_public_key, imagekit_url_endpoint]):
                return json.dumps({
                    ...
                    "message": "ImageKit credentials not found in environment variables"
                })
            ...
            # Upload to ImageKit (using raw binary image data)
            ...
            upload_options = UploadFileRequestOptions(
                ...
                tags=["marketing", "poster", "auto-generated"]
            )
            
            # Generate filename based on timestamp
            timestamp = int(time.time())
            
            # Upload to ImageKit using base64 string (per ImageKit SDK docs)
            upload_result = imagekit.upload_file(
                file=img_base64,
                file_name=f"poster_{timestamp}.png",
            ...
            logger.info("Marketing: Saving metadata to Neon Postgres database")
            ...
            cursor.execute("""
                ...
                VALUES (%s, %s, %s)
            """, (tenant_id, imagekit_url, final_caption))
            
            conn.commit()
            cursor.close()
            conn.close()
            
            logger.info("Marketing: Database record saved successfully")
            ...
            return json.dumps({
                ...
                "message": f"✅ Poster generated and uploaded to ImageKit CDN successfully! Ready to use at: {imagekit_url}"
            })
            
        except ImportError:
            logger.warning("Marketing: Gemini library not installed")
            return json.dumps({
                "success": False,
            ...
            logger.error(f"Marketing: API call failed: {api_error}")
            return json.dumps({
                "success": False,
                "error": "api_error",
        ...
        logger.error(f"Marketing: Unified poster generation/upload failed: {e}")
        return json.dumps({
            "success": False,
            "error": "unified_process_failed",
            "message": str(e)
        })
